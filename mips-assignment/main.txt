...
main:           
    ...
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    sw      $t0,            0($a1)                                  # I-type, Immediate format, store $t0 (smaller) in data_mem
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    sw      $t1,            0($a1)                                  # I-type, Immediate format, store $t1 (larger) in data_mem+4
    j       check_division                                          # J-type, Jump format, jump to check_division

store_large_first:
    ## $t1 is smaller, store $t1 in data_mem and $t0 in data_mem+4
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    sw      $t1,            0($a1)                                  # I-type, Immediate format, store $t1 (smaller) in data_mem
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    sw      $t0,            0($a1)                                  # I-type, Immediate format, store $t0 (larger) in data_mem+4

check_division: 
    ## load integers and check for division by zero
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    lw      $t3,            0($a1)                                  # I-type, Immediate format, load larger integer into $t3
    addi    $a1,            $a1,    -4                              # I-type, Immediate format, subtract 4 from $a1
    lw      $t4,            0($a1)                                  # I-type, Immediate format, load smaller integer into $t4
    ...
    addi    $a1,            $a1,    8                               # I-type, Immediate format, add 8 to $a1
    sw      $t5,            0($a1)                                  # I-type, Immediate format, store quotient in data_mem+8
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    sw      $t6,            0($a1)                                  # I-type, Immediate format, store remainder in data_mem+12
...
handle_div0:    
    ...
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    addi    $a1,            $a1,    8                               # I-type, Immediate format, add 8 to $a1
    sw      $zero,          0($a1)                                  # I-type, Immediate format, store 0 (quotient) in data_mem+8
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    sw      $zero,          0($a1)                                  # I-type, Immediate format, store 0 (remainder) in data_mem+12
    addi    $v0,            $zero,  4                               # I-type, Immediate format, load print string syscall into $v0
...
print_results:  
    ...
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    addi    $a1,            $a1,    4                               # I-type, Immediate format, add 4 to $a1
    lw      $a0,            0($a1)                                  # I-type, Immediate format, load larger integer from memory into $a0
    ...
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    lw      $a0,            0($a1)                                  # I-type, Immediate format, load smaller integer from memory into $a0
    addi    $v0,            $zero,  1                               # I-type, Immediate format, load print integer syscall into $v0
    ...
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    addi    $a1,            $a1,    8                               # I-type, Immediate format, add 8 to $a1
    lw      $a0,            0($a1)                                  # I-type, Immediate format, load quotient from memory into $a0
    addi    $v0,            $zero,  1                               # I-type, Immediate format, load print integer syscall into $v0
    ...
    ## print the remainder
    lui     $a1,            0x1001                                  # I-type, Immediate format, load upper half of data_mem address into $a1
    addi    $a1,            $a1,    12                              # I-type, Immediate format, add 12 to $a1
    lw      $a0,            0($a1)                                  # I-type, Immediate format, load remainder from memory into $a0
    addi    $v0,            $zero,  1                               # I-type, Immediate format, load print integer syscall into $v0
